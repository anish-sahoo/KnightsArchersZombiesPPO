...
def prepare_batch(trajectory, sequence_length, critic):
    ...
    for i in range(num_chunks):
        ...
        batch = {
            'observations': {agent: agent_observations[agent][start_idx:end_idx] 
                           for agent in agents},
            'actions': {agent: agent_actions[agent][start_idx:end_idx] 
                       for agent in agents},
            'old_log_probs': {agent: agent_old_log_probs[agent][start_idx:end_idx] 
                             for agent in agents},
            'returns': {agent: agent_returns[agent][start_idx:end_idx] 
                       for agent in agents},
            'advantages': {agent: agent_advantages[agent][start_idx:end_idx] 
                         for agent in agents},
            'initial_hidden_states_actor': {agent: torch.zeros(1, 1, critic.rnn.hidden_size) 
                                          for agent in agents},
            'initial_hidden_states_critic': {agent: torch.zeros(1, 1, critic.rnn.hidden_size) 
                                           for agent in agents}
        }
...
def update_network(batch, actor, critic, actor_optimizer, critic_optimizer, 
                  hidden_dim, device="cuda", clip_param=0.2, entropy_coef=0.01, 
                  value_loss_coef=0.5, max_grad_norm=0.5):
    """
    Update actor and critic networks using PPO algorithm with RNN support
    """
    # Initialize hidden states
    hidden_actor = {agent: torch.zeros(1, 1, hidden_dim).to(device) 
                   for agent in batch['observations'].keys()}
    hidden_critic = {agent: torch.zeros(1, 1, hidden_dim).to(device) 
                    for agent in batch['observations'].keys()}
    ...
    for agent_id in batch['observations'].keys():
        ...
        obs = torch.tensor(batch['observations'][agent_id], dtype=torch.float32).to(device)
        actions = torch.tensor(batch['actions'][agent_id], dtype=torch.long).to(device)
        old_log_probs = torch.tensor(batch['old_log_probs'][agent_id], dtype=torch.float32).to(device)
        advantages = torch.tensor(batch['advantages'][agent_id], dtype=torch.float32).to(device)
        returns = torch.tensor(batch['returns'][agent_id], dtype=torch.float32).to(device)
        ...
        value, hidden_critic[agent_id] = critic(obs.unsqueeze(0), hidden_critic[agent_id])
...
def train_recurrent_mappo(env, max_timesteps, sequence_length, hidden_dim, device="cuda"):
    # Training variables
    global_timesteps = 0
    episode_count = 0
    episode_rewards = []
    ...
    while global_timesteps < max_timesteps:
        ...
        while True:
            ...
            for agent_id, obs in observations.items():
                ...
                logits, hidden_actor[agent_id] = actor(obs_tensor, hidden_actor[agent_id])
                probs = torch.softmax(logits, dim=-1)
                dist = Categorical(probs)
                action = dist.sample()
            ...
            # Store transition
            trajectory.append((observations, actions, old_log_probs, rewards, 
        ...
        for batch in batches:
            losses = update_network(
                batch, actor, critic, actor_optimizer, critic_optimizer, 
                hidden_dim, device
            )
