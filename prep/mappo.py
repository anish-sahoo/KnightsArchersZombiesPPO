...
class MAPPO:
    def __init__(self, env, hidden_dim=64, lr=3e-3, gamma=0.99, epsilon=0.2):
        ...
        for agent_type in agent_types:
            self.actors[agent_type] = ActorNetwork(obs_dim, hidden_dim, act_dim).to(
                device
            )
            self.critics[agent_type] = CriticNetwork(obs_dim, hidden_dim).to(device)
            self.actor_optimizers[agent_type] = optim.Adam(
                self.actors[agent_type].parameters(), lr=lr
            )
            self.critic_optimizers[agent_type] = optim.Adam(
                self.critics[agent_type].parameters(), lr=lr
            )
    ...
    def train_step(self, trajectories):
        for agent_type in self.actors.keys():
            ...
            # Prepare batch data
            states = torch.stack(
                [self.process_observation(t["state"]) for t in agent_trajectories]
            ).to(device)
            actions = torch.LongTensor([t["action"] for t in agent_trajectories]).to(
                device
            )
            rewards = torch.FloatTensor([t["reward"] for t in agent_trajectories]).to(
                device
            )
            next_states = torch.stack(
                [self.process_observation(t["next_state"]) for t in agent_trajectories]
            ).to(device)
            old_log_probs = torch.FloatTensor(
                [t["log_prob"] for t in agent_trajectories]
            ).to(device)
            ...
            value_loss = F.mse_loss(
                self.critics[agent_type](states).squeeze(),
                rewards + self.gamma * next_values,
            )
...
def main():
    ...
    mappo = MAPPO(env)
    rewards, knight_r, archer_r, actor_l, critic_l = mappo.train(
        num_episodes=20000, max_steps=20000
    )
