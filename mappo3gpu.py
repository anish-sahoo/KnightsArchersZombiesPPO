...
def plot_losses_and_rewards(actor_losses, critic_losses, episode_rewards, iter=None, save=True):
    ...
    while step <= stepmax:
        ...
        for _ in range(batch_size):
            ...
            for t in range(T):
                ...
                rewards = {agent: reward*10 if reward > 0 else -0.1 for agent, reward in rewards.items()}
            ...
            for step_idx in range(0, len(trajectory), k):
                ...
                advantages = []
                gae = 0
                for reward, done, value in zip(reversed(rewards), reversed(dones), reversed(returns)):
        ...
        with tqdm(total=min(minibatch_count, len(D)), desc="Minibatch Updates", leave=False) as mb_pbar:
            for _ in range(min(minibatch_count, len(D))):
                ...
                obs = torch.tensor(np.array(data["obs"]), dtype=torch.float32).to(device)
        ...
        if step % 100 == 0:
            plot_losses_and_rewards(np.array(actor_losses), np.array(critic_losses), np.array(episode_rewards), iter=step, save=True)
