    ...
    def plot_metrics_with_confidence_intervals(self, save_dir='./metrics', save_individual=False):
        ...
        if save_individual:
            for metric_name, values in self.metrics.items():
                ...
                if values.ndim == 2:
                    ...
                    plt.fill_between(range(len(mean)), mean - 1.96 * stderr, mean + 1.96 * stderr, alpha=0.3)
                    plt.legend()
                elif values.ndim == 1:
        ...
        for i, (metric_name, values) in enumerate(self.metrics.items()):
            ...
            if values.ndim == 2:
                ...
                ax.fill_between(range(len(mean)), mean - 1.96 * stderr, mean + 1.96 * stderr, alpha=0.3)
                ax.legend()
            elif values.ndim == 1:
    ...
    def plot_metrics_smooth(self, save_dir='./metrics', save_individual=False, smoothing_window=10):
        ...
        if save_individual:
            for metric_name, values in self.metrics.items():
                ...
                plt.plot(range(len(smoothed_values)), smoothed_values, label='Smoothed', color='orange')
                plt.title(f'{metric_name.replace("_", " ").title()}')
                plt.xlabel('Episode')
                plt.ylabel('Value')
                plt.legend()
        ...
        for i, (metric_name, values) in enumerate(self.metrics.items()):
            ...
            ax.plot(range(len(smoothed_values)), smoothed_values, label='Smoothed', color='orange')
            ax.set_title(metric_name.replace('_', ' ').title())
            ax.set_xlabel('Episode')
            ax.set_ylabel('Value')
            ax.legend()
